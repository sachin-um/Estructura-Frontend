...
const AllRenters = () => {
  ...
  const TabedData = allRentersData
  ...
  const handleSortingOptionChange = (sortingValue) => {
    ...
    switch (sortingValue) {
      case 10: // Price: Low to High
        ...
        allRentersData.sort((a, b) => parseFloat(a.price.replace(',', '')) - parseFloat(b.price.replace(',', '')));
        break;
      case 20: // Price: High to Low
        console.log("came to 20");
        allRentersData.sort((a, b) => parseFloat(b.price.replace(',', '')) - parseFloat(a.price.replace(',', '')));
        break;
      case 30: // Date: Newest on Top
        console.log("came to 30");
        allRentersData.sort((a, b) => new Date(b.datePosted) - new Date(a.datePosted));
        break;
      case 40: // Date: Oldest on Top
        console.log("came to 40");
        allRentersData.sort((a, b) => new Date(a.datePosted) - new Date(b.datePosted));
        break;
      default:
        break;
    }
  ...
  const filterDataByLocation = () => {
    if (locationOption === 'islandwide') {
      ...
      const filteredItems = TabedData.filter((item) => selectedDistricts.includes(item.location));
    ...
    <>
      ...
      <div style={{ margin: "20px", display: "flex", justifyContent: "flex-end" }}>
        <button
          ...
          style={{
            marginRight: "15px",
            backgroundColor: selectedTab === "Heavy Machinery" ? "#804000" : "transparent",
        ...
        <button
          ...
          style={{
            marginRight: "15px",
            backgroundColor: selectedTab === "Portable Machines" ? "#804000" : "transparent",
        ...
        <button
          ...
          style={{
            backgroundColor:
              selectedTab === "Equipment and Power Tools" ? "#804000" : "transparent",
            color: selectedTab === "Equipment and Power Tools" ? "#fff" : "#804000",
            border: "1px solid #804000",
            borderRadius: "4px",
            padding: "5px 10px",
            cursor: "pointer",
